...
function makeup.new(construct, ...)
    ...
    -- local defaultVec = Amaz.Vector()
    -- local defaultMap = Amaz.Map()
    -- defaultMap:set(FACE_ID, -1)
    -- defaultMap:set(FACE_ADJUST_INTENSITY, 1)
    -- defaultVec:pushBack(defaultMap)
    -- self.faceAdjustMaps['face_adjust_eyelash_mashenggan'] = defaultVec
    -- local defaultVec2 = Amaz.Vector()
    -- local defaultMap2 = Amaz.Map()
    -- defaultMap2:set(FACE_ID, -1)
    -- defaultMap2:set(FACE_ADJUST_INTENSITY, 1)
    -- defaultVec2:pushBack(defaultMap2)
    -- self.faceAdjustMaps['face_adjust_lip_liangzhe'] = defaultVec2
    -- local defaultVec3 = Amaz.Vector()
    -- local defaultMap3 = Amaz.Map()
    -- defaultMap3:set(FACE_ID, -1)
    -- defaultMap3:set(FACE_ADJUST_INTENSITY, 1)
    -- defaultVec3:pushBack(defaultMap3)
    -- self.faceAdjustMaps['face_adjust_eyeshadow_dadizhong'] = defaultVec3
...
function makeup:onUpdate(comp, deltaTime)
    ...
    for tagName, entityName in pairs(nameMap) do
        ...
        if entity ~= nil then
            ...
            local table = entity['table']
            if makeup ~= nil and renderer ~= nil and table ~= nil then
                local materials = Amaz.Vector()
                local defaultMaterial = table.table:get('default')
                local lastMaterials = renderer.sharedMaterials
                for i = 0, self.maxFaceNum - 1 do
                    local params = tempIntensity[entityName][i]
                    local matName = params['material']
                    local lastMatName = self.lastTempIntensity[entityName][i]['material']
                    if matName ~= nil then
                        if matName == lastMatName and lastMaterials:size() > i then
                            local material = lastMaterials:get(i)
                            materials:pushBack(material)
                            -- Amaz.LOGW(self.logTag, "onUpdate lastMaterials")
                        else
                            local material = table.table:get(matName)
                            if material ~= nil then
                                -- Amaz.LOGW(self.logTag, "onUpdate matName " .. matName)
