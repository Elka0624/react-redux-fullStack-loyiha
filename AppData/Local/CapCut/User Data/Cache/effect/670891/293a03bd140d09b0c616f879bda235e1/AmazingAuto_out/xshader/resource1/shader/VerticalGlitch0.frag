...
void main() {
  vec2 p = uv0;
  vec4 color1 = texture2D(inputImageTexture, p);
  vec4 color2 = texture2D(inputImageTexture2, p);
  // float progress = clamp(timer, 0.0, 1.0);
  vec2 disp = displace(color1, p, 0.33, 0.7, 1.0-ease1(progress));
  vec2 disp2 = displace(color2, p, 0.33, 0.5, ease2(progress));
  vec4 dColor1 = texture2D(inputImageTexture2, disp);
  vec4 dColor2 = texture2D(inputImageTexture, disp2);
