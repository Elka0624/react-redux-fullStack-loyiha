...
function Transform.new(construct, ...)
    ...
    self.duration = 0
    -- 设置起始点
    self.startPoint = Amaz.Vector3f(0, 0.0, 0.0)
    -- 设置起始点旋转方向
    self.startRotate = Amaz.Vector3f(0.0, 0.0, 0.0)
    -- 设置起始点缩放系数
    self.startScale = Amaz.Vector3f(2.0, 2.0, 1.0)
    -- 设置结束点
    ...
    -- 设置模糊强度
    self.blurIntensity = 0.2
    -- 设置模糊的方向
    self.blurDirection = Amaz.Vector2f(0, 0);
...
local function funcEaseAction(t, b, c, d)
    t=t/d
...
local function funcEaseBlurAction(t, b, c, d)
    ...
    return 0.4*(math.max(1-t,0.4)-0.4)
    --return b + c * t
end
